...
int inputPrimary(struct tm gs0[])
{
	...
	char pstate[120];
	char ny0[100]="前提条件";
	char ny1[100]="原命题的逆否命题";
	char ny2[100]="双条件导出的条件式";
	char ny3[100]="析取式转换为条件式";
	char ny4[100]="合取的定义";
	char ny5[100]="双否定得到"; 
		...
		else
		{
			for(j=0;j<nLen;j++)
			{
				if(pstate[j]=='-')//标注条件式p-q
				{
					...
					gs0[i].gsLast[0]='!';
					for(k=0;k<j;k++)
					gs0[i].gsLast[k+1]=pstate[k];//条件前件进gsLast
					gs0[i].gsLast[k+1]='\0';
				...
				else if(pstate[j]=='=')//标注条件式p=q
				{
					...
					gs0[i].nLitter=gs0[i-1].nLitter;
					for(k=0;k<j;k++)
					gs0[i].gsLast[k]=pstate[k];//条件前件进gs
					gs0[i].gsLast[k]='\0';
					for(k=j+1;k<nLen;k++)
					gs0[i].gs[k-j-1]=pstate[k];//条件后件进gsLast
					gs0[i].gs[k-j-1]='\0';
				...
				else if(pstate[j]=='+')//标注条件式p+q
				{
					...
					gs0[i].gsLast[k-j-1]=pstate[k];//q进gsLast
					gs0[i].gsLast[k-j-1]='\0';
					setNiyou(gs0,i,ny3,i-1,-1,0,1,0);//前提类型，无，无，未使用，是条件式，不是文字
					nonoop2(gs0[i].gs);
					//!q-p
					...
					gs0[i].gs[0]='!';
					for(k=j+1;k<nLen;k++)
					gs0[i].gs[k-j-1+1]=pstate[k];//条件后件进gsLast
					gs0[i].gs[k-j-1+1]='\0';
					setNiyou(gs0,i,ny3,i-2,-1,0,1,0);//前提类型，无，无，未使用，是条件式，不是文字
					nonoop2(gs0[i].gs);
					break; 
				}
				else if(pstate[j]=='*')
				{
					...
					gs0[i].gs[1]='\0';
					gs0[i].gsLast[0]='\0';
					setNiyou(gs0,i,ny4,i-1,-1,0,0,1);
					i++;
					gs0[i].nLitter=1;
					gs0[i].gs[0]=pstate[2];
					gs0[i].gs[1]='\0';
					gs0[i].gsLast[0]='\0';
					setNiyou(gs0,i,ny4,i-2,-1,0,0,1);
					break; 
				}
			}
...
int main()
{
	...
	char ny01[100]="假言推理";
	int i=0,k=0,j=0,np=1,np0=0,isOk=0;
	int i0=0,nPosLitter[128]={0},nPosCond=0;//文字起始位置，首个文字的位置，条件的位置
	//输入前提条件
	...
	while(1)
	{
		...
		nPosLitter[j]=i;//记录文字的位置 
		strcpy(stmp[j],tmrec[i].gs);//保存当前文字的内容
		if(strcmp(result0,stmp[j])==0)//推出结论同条原始条件的下一轮 
		{
			isOk=1;
			break;
		} 
			...
			while(i<np) //若找到了这样的条件 
			{
				if(tmrec[i].isCond==1)//若是条件式
				{
					...
					if((strcmp(lastNiYou,tmrec[i].niyou)!=0)||((strcmp(lastNiYou,tmrec[i].niyou)==0)&&(tmrec[i].niyou[0]!='(')))
					{
						...
						if(strcmp(tmrec[i].gs,stmp[k])==0)//如果条件式的前件与文字相等
						{
							strcpy(lastNiYou,tmrec[i].niyou);
							//tmrec[nPosLitter].nUsed++;//这个文字用一次了
							tmrec[i].nUsed++;//这个条件用过一次了
							j++;
							strcpy(stmp[j],tmrec[i].gsLast);//将此中间结果保存到推导序列中
							strcpy(tmrec[np].gs,stmp[j]);//将当前推出来的结果保存起来
							tmrec[np].gsLast[0]='\0';//后件清空，保存当前条件
							setNiyou(tmrec,np,ny01,nPosLitter[k],i,0,0,1);//前提类型，有，无，未使用，不是条件式
							nPosLitter[j]=np;//记录当前文字的序号
							np++;
							if(strcmp(result0,stmp[k])==0)//推出结论同条原始条件的下一轮 
							{
								isOk=1;
								break;
							} 
